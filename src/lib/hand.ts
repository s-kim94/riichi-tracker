...
export type Meld =
  | { t: "chiipon"; tiles: TileCode[] }
  | { t: "kan"; closed: boolean; tiles: TileCode[] };
...
export function sortTiles(tiles: TileCode[]): TileCode[] {
  ...
  return tiles.sort((x, y) => {
    ...
    return (
      suitOrder[sx] - suitOrder[sy] ||
      (Number(x[0]) || 4.9) - (Number(y[0]) || 4.9)
    );
  });
}
...
export function sortMelds(melds: Meld[]): Meld[] {
  ...
  return melds.sort((x, y) => {
    ...
    return (
      suitOrder[sx] - suitOrder[sy] ||
      (Number(x.tiles[0][0]) || 4.9) - (Number(y.tiles[0][0]) || 4.9)
    );
  });
}
...
export function calculateScoreTable(
  ...
  return {
    tsumoAsFromOya:
      ceil100(base * 2) +
      (settings.sanma === "bisection" ? ceil100(ceil100(base * 2) / 2) : 0),
    tsumoAsKo:
      ceil100(base) +
      (settings.sanma === "bisection" ? ceil100(ceil100(base) / 2) : 0),
...
export function makeScore(
  ...
  return agari === "ron"
    ...
    : {
        ...
        points: {
          total: isOya
            ? tsumoAsFromOya * (sanma ? 2 : 3)
            : tsumoAsFromOya + tsumoAsKo * (sanma ? 1 : 2),
...
export function possibleHanFuValues(
  agari: "tsumo" | "ron",
): Map<number, number[]> {
...
export function convertHand(hand: Hand, sanma: boolean): string {
  let s = "";
  const suits = partitionSuits(
    hand.tiles.filter((_t, i) => i !== hand.agariIndex),
  );
  ...
  if (hand.dora.length) {
    s += "+d";
    const doraSuits = partitionSuits(
      hand.dora.map((x) => nextDoraTile(x, 1, sanma)),
    );
  ...
  if (hand.uradora.length) {
    s += "+u";
    const doraSuits = partitionSuits(
      hand.uradora.map((x) => nextDoraTile(x, 1, sanma)),
    );
    for (const [suit, tiles] of doraSuits) {
      s += tiles;
      s += suit;
